...
#+author: AmberWing
#+PROPERTY: header-args:emacs-lisp :tangle ~/.emacs.amberwing.d/init.el :makedirp yes

* Instructions

If this is the first time, you may have to run ~M-x org-babel-tangle~ to have it actually run. After that, saving the file /should/ automatically tangle it.
...
In addition, you will need to install the package ~nerd-icons~ and run ~M-x nerd-icons-install-fonts~.
...
#+begin_src emacs-lisp :tangle ~/.emacs.amberwing.d/ambria-theme.el
...
#+begin_src emacs-lisp :tangle ~/.emacs.amberwing.d/ambria-light-theme.el
...
#+begin_src emacs-lisp
  ...
  (setq custom-theme-directory "~/.emacs.amberwing.d")
